...
Create datatable to provide staff's details by name, department, position, citizenship country, and military service:
...
Process name column format in "employee_backgrounds" file to match employee names in GasTech_nodes. The latter has a "." between first and midd/last names.

```{r echo=FALSE}
employee_backgrounds$Name <- gsub(" ", ".", employee_backgrounds$Name, fixed=TRUE)
```

Merge the 2 files and remove unnecessary columns which cannot fit into the interactive datatable below.
...
By inputting the "POK" in the search tab, all GASTech staff with POK affiliations would be automatically retrieved. In total, there are 5 POK direct members, namely, Minke.Mies, Isia.Vann, Loreto.Bodrogi, Hennie.Osvaldo. and Inga.Ferro. To extract all other personnel with external profiles, click on the upward arrow at the "POK/Others" column and you could see all other suspicious member profiles such as Edward.Vann who could be related to the Vann members of POK as well as Ruscella.Mies.Haber (page2) who is aunt of Minke.Mies.



### 3.4.2 Identify key email personnels (nodes) by job title:
