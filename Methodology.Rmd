...
## A(i) Installation and loading of required packages:
...
## A(ii) Import & merge data

Function is create to read and collect all the news articles into a dataframe:
...
The created dataframe on news text is then saved to rds file to minimize memory consumption:
...
## A(iii) Clean & merge data

The news text is further processed to provide columns for title, date and location which would be extracted from the text column containing the news text:
...
Note that although title column is created and copied from the text column, the text column still retain the title of the article for purposes of subsequent processing and analysis. However, location and date published would be removed after it is extracted into separate columns as these 2 details should not be include in the analysis and further processing of the main news text.
...
## 1.1 Understand Parameters of the News Articles - Frequency & Duration
...
## 1.2 Further text cleaning to obtain stem words
...
Text is tokenized and stopwords were removed:
...
Remove meaningless and frequent words not removed in stopwords - title, published & kronos - so that they do not distort the subsequent text visualization:
...
## 1.3 Plot the newsgroups by sentiment scores of their articles to detect similar profiles 
...
From above, there are indeed a few pairs/groups of common sentiment score profiles, e.g. The Abila Post and Central Bulletin had same sentiment scores. Could these pairs/groups be posting common or similar news articles resulting in the same scores? If so, which is the primary and derivative sources? We seek to find out in the following analysis.
...
## 1.4 To detect important words by newsgroup using TFIDF

```{r echo=TRUE, eval=TRUE, layout="l-body-outset", fig.width=16, fig.height=10}
tf_idf %>%
  group_by(newsgroup) %>%
...
The above shows some newsgroup with common profiles in terms of TFIDF. This would be investigated further below by reviewing the correlation of the news articles.


## 1.4 To identify news articles with high correlation

First, pairwise correlation score is calculated by article text of each newsgroup:
...
### The above chart clearly visualizes newsgroup with high correlation of the news articles. There are nodes with thicker edges, such as Kronos Star and International News, show the higher correlation of their news articles.
