...
```{r echo=TRUE, eval=TRUE, layout="l-body-outset", fig.width=18, fig.height=15}
tf_idf %>%
  group_by(newsgroup) %>%
...
```{r, layout="l-body-outset", fig.width=14, fig.height=9}
g <- GAStech_graph %>%
  ggraph(layout = "fr") + 
...
g <- GAStech_graph %>%
  ...
  geom_node_point(aes(colour = CitizenshipCountry, 
                      size = centrality_betweenness()))+
  ggtitle("Nodes & Edges of Email Network - Citizenship Country")
